...
#### ComponentResponse
Komponenty zwracają `ComponentResponse` (`src/core/component.rs`) z informacją:

- czy klawisz został obsłużony,
- jakie wartości komponent wyprodukował (`produced`),
- jakie inputy mają zostać zaktualizowane (`changes`),
- czy został zgłoszony submit (`submit_requested`).

To jest główny kanał komunikacji między komponentem a `FormEngine`.
...
- `KeyBindings` (`src/core/key_bindings.rs`) mapuje skróty (Tab, Shift+Tab, Ctrl+Backspace, itp.) na `Command`.

Obsługa klawiszy:

1. Jeśli klawisz pasuje do globalnego `KeyBindings`, jest mapowany na `Command`.
2. W pozostałych przypadkach klawisz trafia do aktualnie sfokusowanego inputu/komponentu przez `FormEngine`.

### 3.4 Reducer and Effects

- `Reducer` (`src/core/reducer.rs`) is the main state transition engine.
- It processes `Command` and returns `Effect` objects:
...
### 3.5 Form Engine (Focus + Input Changes)
...
`FormEngine::handle_key` returns `EngineOutput`:

- `input_changes`: zmienione wartości inputów,
- `produced`: wartości emitowane przez komponenty,
- `handled`: czy klawisz został obsłużony,
- `submit_requested`: czy wejście poprosiło o submit.

### 3.6 Component‑Driven Input Changes

`ComponentResponse` może zawierać `changes`, co pozwala **group components** aktualizować child inputy przez `FormEngine` bez bezpośredniego ustawiania focusu na dziecku.
...
`RenderContext` (`src/ui/render/render_context.rs`) provides access to theme and rendering helpers:
...
3. `Tab` is handled as `Command::TabKey` in the `Reducer` to allow autocomplete‑then‑focus logic.
4. Remaining keys become `Command::InputKey` and go through `Reducer`.
5. `FormEngine` mutates focused node and produces `EngineOutput`.
6. `Reducer` converts output into `Effect` objects.
7. `App::apply_effects` emits events and updates state.
8. Rendering uses the updated state tree.
...
- `src/core/component.rs`: component base, focus mode, response model, delete‑word hooks.
...
### UI
- `src/ui/render/render_context.rs`: render context + input rendering helpers.
- `src/ui/render/step_render.rs`: step/node trait rendering.
...
- **Focus group**: set `FocusMode::Group` and return `ComponentResponse` with input changes/produced values.
