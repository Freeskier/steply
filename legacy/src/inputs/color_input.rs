...
impl ColorInput {
    ...
    fn handle_hex_digit(&mut self, ch: char) -> bool {
        ...
        let value = if self.edit_buffer.len() == 1 {
            ...
            parse_hex_pair_prefix(&self.edit_buffer).unwrap_or(digit * 16)
        };
    ...
    fn handle_rgb_digit(&mut self, ch: char) -> bool {
        ...
        let value = parse_clamped_u8(&self.edit_buffer, 255);
        let idx = self.channel.index();
        self.rgb[idx] = value;
        self.base.error = None;
        true
    ...
    fn handle_backspace(&mut self) -> bool {
        ...
        match self.mode {
            ColorMode::Hex => {
                let value = parse_hex_high_nibble(&self.edit_buffer).unwrap_or(0);
                let idx = self.channel.index();
                self.rgb[idx] = value;
            }
            ColorMode::Rgb => {
    ...
    fn set_hsl_channel_from_buffer(&mut self) {
        let mut hsl = rgb_to_hsl(self.rgb);
        match self.channel {
            Channel::First => hsl[0] = parse_clamped_u16(&self.edit_buffer, 360),
            Channel::Second => hsl[1] = parse_clamped_u16(&self.edit_buffer, 100),
            Channel::Third => hsl[2] = parse_clamped_u16(&self.edit_buffer, 100),
        }
        self.rgb = hsl_to_rgb(hsl);
    }
