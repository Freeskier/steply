...
impl PasswordInput {
    ...
    fn raw_len(&self) -> usize {
        text_edit::char_count(&self.inner.raw_value())
    }

    fn mask_char(&self) -> char {
        match self.render_mode {
            PasswordRender::Stars => '*',
            PasswordRender::Hidden => ' ',
        }
    }
}
...
impl Input for PasswordInput {
    ...
    fn render_content(&self, _theme: &crate::theme::Theme) -> Vec<Span> {
        let text = std::iter::repeat(self.mask_char())
            .take(self.raw_len())
            .collect::<String>();
        vec![Span::new(text)]
    }
    ...
    fn cursor_offset_in_content(&self) -> usize {
        self.inner.cursor_pos().min(self.raw_len())
    }
