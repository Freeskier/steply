...
impl PathInput {
    ...
    fn autocomplete(&mut self) -> bool {
        ...
        let Some(mut matches) = self.collect_matches(dir, &prefix) else {
            return false;
        };
        ...
        if matches.len() == 1 && matches[0].0 == prefix {
            let Some(mut all_matches) = self.collect_matches(Path::new(&dir_path), "") else {
                return false;
            };
    ...
    fn collect_matches(&self, dir: &Path, prefix: &str) -> Option<Vec<(String, bool)>> {
