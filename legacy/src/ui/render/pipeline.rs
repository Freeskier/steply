...
impl RenderPipeline {
    ...
    pub fn render_layer(
        ...
        let mut start_row = self.layer_start_row(anchor_cursor);
        ...
        if let Some(region) = &self.region {
            ...
            if desired > region.line_count {
                let before_start = region.start_row;
                let _ = self.ensure_region(terminal, desired)?;
                let after_start = self
                    .region
                    .as_ref()
                    .map(|updated| updated.start_row)
                    .unwrap_or(before_start);
                let scrolled = before_start.saturating_sub(after_start);
                if scrolled > 0 {
                    start_row = start_row.saturating_sub(scrolled);
                }
            }
        }
    ...
    fn ensure_region(&mut self, terminal: &mut Terminal, line_count: usize) -> io::Result<u16> {
        if let Some(region) = self.region.as_ref() {
            if line_count > region.line_count {
                ...
                let end_row = region.start_row.saturating_add(region.line_count as u16);
                terminal.queue_move_cursor(0, end_row)?;
                for _ in 0..extra {
                    writeln!(terminal.writer_mut())?;
                }
                ...
                terminal.refresh_size()?;
                let scrolled = Self::scrolled_lines(terminal.size().height, end_row, extra);
                self.shift_cached_regions_up(scrolled);

                if let Some(region) = &mut self.region {
                    region.line_count = line_count;
                }
            }
            return Ok(self.region.as_ref().map(|r| r.start_row).unwrap_or(0));
        }
    ...
    fn layer_start_row(&self, anchor_cursor: Option<(u16, u16)>) -> u16 {
        anchor_cursor
            .map(|(_, row)| row.saturating_add(1))
            .or_else(|| {
                self.region
                    .as_ref()
                    .map(|r| r.start_row.saturating_add(r.line_count as u16))
            })
            .unwrap_or(0)
    }

    fn shift_cached_regions_up(&mut self, lines: u16) {
        if lines == 0 {
            return;
        }

        if let Some(region) = &mut self.region {
            region.start_row = region.start_row.saturating_sub(lines);
        }
        if let Some(layer_region) = &mut self.layer_region {
            layer_region.start_row = layer_region.start_row.saturating_sub(lines);
        }
    }

    fn scrolled_lines(height: u16, end_row: u16, appended_lines: usize) -> u16 {
        let bottom_row = height.saturating_sub(1) as usize;
        let expected_row = end_row as usize + appended_lines;
        let scrolled = expected_row.saturating_sub(bottom_row);
        scrolled.min(u16::MAX as usize) as u16
    }
