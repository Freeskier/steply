...
impl SegmentedInput {
    ...
    fn insert_char(&mut self, ch: char) -> bool {
        ...
        let mut buf: Vec<char> = value.chars().collect();
        let pos = cursor_offset.min(buf.len());
        buf.insert(pos, ch);
        *value = buf.into_iter().collect();
    ...
    fn delete_prev(&mut self) -> bool {
        ...
        if let Some(prev) = Self::prev_segment_pos(&self.tokens, token_idx) {
            ...
            self.cursor_offset = match self.current_segment() {
                Some(SegmentToken::Segment { value, .. }) => value.chars().count(),
                _ => 0,
            };
    ...
    fn move_right(&mut self) -> bool {
        if let Some(SegmentToken::Segment { value, .. }) = self.current_segment() {
            if self.cursor_offset < value.chars().count() {
                self.cursor_offset += 1;
                return true;
            }
        }
    ...
    fn increment_current(&mut self, delta: i64) -> bool {
        ...
        self.cursor_offset = value.chars().count();
        true
    }
                ...
                SegmentToken::Segment {
                    ...
                    if value.is_empty() {
                        ...
                        if let Some(ph) = placeholder {
                            if let Some(max) = max_len {
                                if min_len == max {
                                    let pad = max.saturating_sub(value.chars().count());
                                    for _ in 0..pad {
                                        out.push(ph);
                                    }
                                }
        ...
        for (token_idx, token) in self.tokens.iter_mut().enumerate() {
            match token {
                SegmentToken::Literal(ch) => {
                    if value.chars().nth(idx) != Some(*ch) {
                        return;
                    }
                    idx += 1;
                }
                SegmentToken::Segment {
                    ...
                    while idx < value.len() {
                        let ch = value.chars().nth(idx).unwrap();
