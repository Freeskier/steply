...
impl From<TextInput> for CustomValueInput {
    fn from(value: TextInput) -> Self {
        Self::Text(value)
    }
}

impl From<MaskedInput> for CustomValueInput {
    fn from(value: MaskedInput) -> Self {
        Self::Masked(value)
    }
}
...
impl InlineKeyValueEditor {
    ...
    pub fn new_custom(
        id: impl Into<String>,
        _label: impl Into<String>,
        value_input: CustomValueInput,
    ) -> Self {
        let id = id.into();
        let value_field = match value_input {
            CustomValueInput::Text(input) => InlineValueField::Text(input),
    ...
    pub fn with_default_value(mut self, value: impl Into<String>) -> Self {
        ...
        match &mut self.value_field {
            ...
            InlineValueField::Masked(masked) => masked.set_value(value),
        }
        self
    }
    ...
    pub fn value_text(&self) -> String {
        match &self.value_field {
            ...
            InlineValueField::Text(text) => text
                ...
                .and_then(|value| value.to_text_scalar())
                .unwrap_or_default(),
            InlineValueField::Masked(masked) => masked
                .value()
                .and_then(|value| value.to_text_scalar())
                .unwrap_or_default(),
        }
    ...
    pub fn on_text_action(&mut self, action: TextAction) -> InteractionResult {
        match self.focus {
            ...
            InlineKeyValueFocus::Value => match &mut self.value_field {
                ...
                InlineValueField::Masked(_) => InteractionResult::ignored(),
            },
        }
    }
    ...
    pub fn inline_spans(&self) -> Vec<Span> {
        ...
        match &self.value_field {
            InlineValueField::Select(_) => {
                out.push(Span::styled(format!("‹ {} ›", self.value_text()), value_style).no_wrap())
            }
            InlineValueField::Text(_) => {
